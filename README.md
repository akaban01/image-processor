...
js/lib/
  base64.js               base64 encoding and the paste-ready snippets
...
**Base64 text** — every converted image has a **Base64** button that opens its text
form: a data URL, the bare base64, an `<img>` tag with the output dimensions filled in, a
CSS `background-image` declaration, or Markdown. Copy it, or save it as a text file
alongside the image. The wrapping is a setting, so the one you use is the one that opens
next time. Encoding happens once per image and the text is capped at 200,000 characters
on screen — copy and save always give the whole thing.
...
photo standards and their compliance checks, JPEG density stamping, base64 encoding and
its snippets, batch concurrency and cancellation, and the ZIP writer — whose output is additionally verified
